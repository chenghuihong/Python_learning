import pygame as pg  # 用于创建游戏窗口、处理游戏逻辑和绘制图形
import random as ra  # 用于生成随机数
import math  # 用于数学计算

pg.init()  # 初始化Pygame
pg.display.set_caption("🎇")  # 设置窗口标题
...
class Firework:
    ...
    def update(self, win):  # 更新烟花的状态、含烟花运动和爆炸效果
    ...
    def explode(self):  # 触发烟花的爆炸效果、生成爆炸产生的粒子对象
...
class Trail:
    ...
    def get_pos(self, x, y):  # 更新痕迹位置
        self.pos = vector(x, y)

    def show(self, win):  # 在游戏窗口绘制痕迹
        # 绘制痕迹
        pg.draw.circle(win, self.colour, (int(self.pos.x), int(self.pos.y)), self.size)


def update(win, fireworks):  # 更新游戏窗口中的烟花效果
    for fw in fireworks:
        fw.update(win)
        if fw.remove():
            fireworks.remove(fw)
...
def fire():  # 用于开始游戏并创建游戏窗口，处理游戏的主循环和事件处理
...
if __name__ == "__main__":  # 调用fire函数，开始发射烟花
    fire()
